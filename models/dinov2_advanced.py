...
class DinoV2Finetune(nn.Module):
    ...
    #  lora_r_vision=16, lora_alpha_vision=32, lora_dropout_vision=0.05,
     ...
     coef_1=0.5, coef_2=0.5, hidden_dim_1=512, hidden_dim_2=256,vision_coef=0.5,use_concatenation=True,
     dropout_p=0.2, init_method="kaiming", model_name="efficientnet_b5" , batch_or_layer="batch" ,
      vision_classification_model=None, coef_image_1=0.8):
        super().__init__()
        ...
        self.vision_classification_model=vision_classification_model
        if self.vision_classification_model : 
            self.backbone_2 = timm.create_model(vision_classification_model, pretrained=True)
            self.backbone_2_vision_head = self.backbone_2.backbone
            self.backbone_2_classification_head=self.backbone_2.classification_head #(batch, hidden_dim_2=)
            self.backbone_2_hidden_dim_2=self.backbone_2_classification_head.hidden_dim_2
    ...
    def forward(self, x):
        ...
        # if self.vision_classification_model : 
        #     out_image_2 = self.backbone_2_vision_head(imgs) #[batch, ]
        #     out_image_2=self.backbone_2_classification_head(out_image_2) #[batch, hidden_dim_2]

...
sweep_config_improved = {
    ...
    'parameters': {
        ...
        'vision_model': {'values': ['mobilenetv4_hybrid_medium', 'efficientnet_b4', 'resnet50']},
        ...
        # Training parameters
        'learning_rate': {'values': [2e-3, 5e-4, 1e-3]},
        'epochs': {'values': [10]},
...
def train_improved_model(cfg):
    ...
    with wandb.init(
        ...
        val_years =[2021,2022,2023,2013,2019]
        test_years =[2014,2018,2015]
        for year in val_years:
            cfg.datamodule.val_years = [year]
            cfg.experiment_name = f"{cfg.experiment_name}_val{year}"
            ...
            for epoch in tqdm(range(config.epochs), desc="Epochs"):
                ...
                if epoch_val_loss < best_val_loss:
                    ...
                    patience_counter = 0
                    print(f"New best validation loss_{year}: {best_val_loss:.4f}")
                    run.log({f"best_val_loss_{year}": best_val_loss})
                    # torch.save(model.state_dict(), f"{run.name}_best_model.pt")
                else:
                    patience_counter += 1
            ...
            mean_best_val_loss /= len(val_years)
            run.log({"mean_best_val_loss": mean_best_val_loss})
            print(f"Mean best validation loss: {mean_best_val_loss:.4f}")
            #debut test de validation sur des données deja vu par le model
            print(f" debut test de validation sur des données deja vu par le model")
            for test_year in test_years:
                cfg.datamodule.val_years = [test_year]
                ...
                run.log({f"test/loss_epoch_{str(year)}": epoch_test_loss,f"test/loss_epoch_year{str(year)}": test_year})
                print(f"Epoch {epoch} test loss_{str(year)}: {epoch_test_loss:.4f}")
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(
        sweep_config_improved, 
        project="version3_Challenge_Improved_CrossModal_AntiOverfit"
    )
    wandb.agent(sweep_id, function=train_improved_model, count=30)
